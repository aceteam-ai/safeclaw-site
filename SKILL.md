...
Start with host port 8899, then fall back through 8898, 8897, 8896, 8895. Store the chosen host port as `{HOST_PORT}`. Set `{PROXY_URL}` to `http://localhost:{HOST_PORT}`.
...
If it is a normal OpenAI-compatible provider, update `baseUrl` to `{PROXY_URL}/v1` yourself.

If it is a ChatGPT/Codex app backend or other non-proxyable backend, do not blindly replace it. Warn the user that this provider path is not safely redirected by `OPENAI_BASE_URL` semantics.
